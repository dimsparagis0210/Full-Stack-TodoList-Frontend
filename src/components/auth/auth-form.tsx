...
export const AuthForm = (props: { isSignUp: boolean }) => {
  ...
  const [isLoading, setIsLoading] = useState(false); // State to show/hide loading spinner and disable the form
  ...
  const onSubmit = async (data: SignInData | SignUpData) => {
    ...
    try {
      ...
      if (isSignUp && "name" in data) {
        // Check if the form is sign up and if the data has a name
        const dto: SignUpDTO = {
          name: data.name,
          email: data.email,
