...
export const Task = ({ task }: TaskProps) => {
  ...
  const priorityColor =
    task.priority === "Low"
      ...
      ? "bg-myYellow/10 text-myYellow"
      : "bg-myRed/10 text-myRed"; // Set the priority color
  ...
  const handleMoveTask = (newStatus: string) => {
    ...
    editTaskMutation.mutate(
      { task: newTask, taskId: task.id },
      {
        onSuccess: () => {
          setDropdownOpen(false); // Close the dropdown after successful move
        },
      }
    );
  };

  // When edit is successful, close the dropdown
          ...
          <section className="hover:bg-[#f5f5f5] hover:cursor-pointer transition-all duration-300 flex flex-col gap-y-10 border border-[#E3E3E3] rounded-lg p-5 my-shadow bg-white">
            <header className="flex flex-col gap-y-2">
              <section className="flex justify-between">
                ...
                <h2 className="h-fit text-sm bg-[#27C5C5]/10 text-primary-blue text-white px-2 py-1 rounded-md">
                  {task.category}
                </h2>
              </section>
              <section className="flex w-fit items-center gap-x-2 border border-[#E2E2E2] rounded-sm p-1">
                ...
                <p className="text-sm text-primaryGray">
                  {getDateRange(task.startDate, task.dueDate)}
                </p>
              </section>
            </header>
            <section className={`flex flex-col gap-y-2`}>
              <section
                className={`w-fit ${priorityColor} rounded-sm py-3 px-10 text-lg`}
              >
                <p>{task.priority}</p>
              </section>
              <section className={`flex justify-between items-end`}>
                ...
                <section
                  className={`w-12 aspect-square flex items-center justify-center text-xl text-primaryGray bg-[#E2E2E2] rounded-full`}
                >
                  <p>{assigneeName}</p>
                </section>
              </section>
